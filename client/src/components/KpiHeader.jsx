...
export default function KpiHeader({ kpis, loading }) {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm animate-pulse h-28"
          ></div>
        ))}
      </div>
    );
  ...
  const renderTrend = (change) => {
    if (change === 0)
      return (
        <span className="text-gray-400 text-xs font-medium">No change</span>
      );
    const isPositive = change > 0;
    return (
      <span
        className={`inline-flex items-center gap-0.5 text-xs font-semibold ${isPositive ? "text-green-600" : "text-red-600"}`}
      >
