...
def register_alert(payload: schemas.AlertRegisterRequest, db: Session = Depends(database.get_db)):
    ...
    if card_num not in settings.MOCK_CMS_CARDS:
        raise HTTPException(
            ...
            detail="Card not found in system"
        )
    ...
    return schemas.AlertRegisterResponse(
        otpReferenceId=otp_tx.otp_reference_id,
...
def verify_otp(payload: schemas.OTPVerifyRequest, db: Session = Depends(database.get_db)):
    card_num = payload.cardNumber
    mobile_num = payload.mobileNumber
...
def simulate_spend(payload: schemas.SimulateSpendRequest, db: Session = Depends(database.get_db)):
    card_num = payload.cardNumber
    card_identifier = card_num[-4:]

    alert_rule, breached, sms_message = crud.record_spend(db, card_identifier, payload.amount)
    ...
    return schemas.SimulateSpendResponse(
        ...
        breached=breached,
        sms_sent=breached,
        message=sms_message if breached else "Transaction processed successfully. Spend is within threshold."
    )
...
def create_user(payload: schemas.UserCreateRequest, db: Session = Depends(database.get_db)):
    try:
...
def update_user(user_id: str, payload: schemas.UserUpdateRequest, db: Session = Depends(database.get_db)):
    try:
...
@router.post("/admin/roles", response_model=schemas.RoleBase, status_code=status.HTTP_201_CREATED)
def create_role(payload: schemas.RoleCreateRequest, db: Session = Depends(database.get_db)):
    try:
...
@router.put("/admin/users/{user_id}/roles", response_model=schemas.UserRolesUpdateResponse)
def assign_user_roles(user_id: str, payload: schemas.UserRolesUpdateRequest, db: Session = Depends(database.get_db)):
    try:
...
@router.patch("/admin/users/{user_id}/permissions", response_model=schemas.UserPermissionsUpdateResponse)
def update_user_permissions(user_id: str, payload: schemas.UserPermissionsUpdateRequest, db: Session = Depends(database.get_db)):
    try:
        ...
        return schemas.UserPermissionsUpdateResponse(user_id=user.id, permissions=user.permissions)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
...
@router.patch("/admin/roles/{role_id}/permissions", response_model=schemas.RolePermissionsUpdateResponse)
def update_role_permissions(role_id: str, payload: schemas.RolePermissionsUpdateRequest, db: Session = Depends(database.get_db)):
    try:
        ...
        return schemas.RolePermissionsUpdateResponse(role_id=role.id, permissions=role.permissions)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
...
def get_dashboard_users(
    ...
    users, total = crud.get_dashboard_users(db, skip=skip, limit=limit, search=search, role=role, status=status)
...
@router.get("/admin/dashboard/audit-logs", response_model=schemas.DashboardAuditLogsResponse)
def get_dashboard_audit_logs(
    ...
    logs, total = crud.get_audit_logs(db, skip=skip, limit=limit, action_type=action_type, actor_id=actor_id)
